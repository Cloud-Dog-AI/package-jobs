...
def test_admin_get_cancel_and_status() -> None:
    backend = MemoryQueueBackend()
    queue = JobQueue(backend)
    job_id = queue.submit(JobRequest(job_type="demo"))

    admin = AdminService(backend)
    assert admin.get_job(job_id) is not None
    assert admin.cancel_job(job_id) is True
    assert admin.queue_status().get("cancelled", 0) == 1
