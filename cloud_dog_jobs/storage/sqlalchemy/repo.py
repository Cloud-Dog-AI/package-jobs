...
class SQLAlchemyJobRepository:
    ...
    def record_attempt(self, job_id: str, *, error: str | None = None) -> bool:
        ...
        with self.engine.begin() as conn:
            ...
            result = conn.execute(
                update(self.jobs)
                .where(self.jobs.c.job_id == job_id)
                .values(meta=meta, updated_at=datetime.now(tz=timezone.utc))
            )
        return result.rowcount == 1

    def update_progress(self, job_id: str, progress: dict) -> bool:
        """Persist progress snapshot into job meta."""
        import json
        with self.engine.begin() as conn:
            row = conn.execute(select(self.jobs.c.meta).where(self.jobs.c.job_id == job_id)).mappings().one_or_none()
            if row is None:
    ...
    def store_result(self, job_id: str, result_data: str | dict) -> bool:
        """Store handler result in job meta."""
        import json
        with self.engine.begin() as conn:
            row = conn.execute(select(self.jobs.c.meta).where(self.jobs.c.job_id == job_id)).mappings().one_or_none()
            if row is None:
