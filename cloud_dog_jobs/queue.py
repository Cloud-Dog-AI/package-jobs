...
class JobQueue:
    ...
    def submit(self, request: JobRequest) -> str:
        ...
        Performs cycle detection when a DependencyTracker is configured
        and the request has depends_on edges (PS-95 §6.3, W28D-305).
        """
        ...
        if self._dependency_tracker is not None and job.depends_on:
            self._dependency_tracker.validate_and_register(job.job_id, job.depends_on)
