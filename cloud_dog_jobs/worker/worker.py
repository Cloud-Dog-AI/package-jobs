...
class Worker:
    ...
    When *resource_pool*, *concurrency*, and/or *dependency_tracker* are
    supplied, the worker delegates job selection to :class:`Dispatcher`
    which enforces dependency ordering, per-resource and per-type
    concurrency limits.  Resource slots are released in a ``finally``
    block on every terminal outcome.
    PS-95 §6.1-§6.3 / W28D-304 / W28D-305.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if resource_pool is not None or concurrency is not None or dependency_tracker is not None:
    ...
    def run_once(self) -> bool:
        ...
        When a Dispatcher is configured (via resource_pool/concurrency),
        job selection uses select_eligible() which enforces limits.
        Resource slots are released in a finally block on EVERY outcome.
        """
        ...
        if not self._backend.claim(job.job_id, self._host_id, self._worker_id):
            if self._dispatcher is not None:
                self._dispatcher.release_job(job)
            return False
